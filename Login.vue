<template>
  <div class="pl-login-frame">
    <div class="pl-login-form">
      <div class="is-logo" title="이베스트투자증권"></div>
      <v-text-field
        name="input-10-1"
        placeholder="아이디를 입력하세요"
        class="pl-form is-login is-mb-10"
      ...
      <v-text-field
        ...
        class="pl-form is-login"
       
        v-model="upwd"
        @keyup.enter="getKey"
      ></v-text-field>
      <div class="is-check">
        <v-checkbox class="pl-check" label="CTI 사용" v-model="chk1"></v-checkbox>
        <v-checkbox class="pl-check" label="ID 저장" v-model="chk2"></v-checkbox>
      </div>
      <v-btn class="pl-btn is-xl is-fill" @click="getKey">로그인</v-btn>
      <!-- <span class="login-findpass" @click="checkdialogFindPass"> 비밀번호 변경</span> -->
    </div>
    <div class="pl-link-txt">매뉴얼 다운로드</div>
    <!-- 비밀번호 변경 모달 -->
    <v-dialog
      max-width="400"
      persistent
      content-class="pl-dialog-wrap"
      v-model="dialogFindPass"
    >
      <div class="pl-dialog-header">
          <h1>비밀번호 변경</h1>
          <div class="is-right">
              <v-btn class="pl-btn is-icon" title="닫기" @click="checkdialogFindPass"><i class="pl-icon-close-lg is-white"></i></v-btn>
          </div>
      </div>
      <div class="pl-dialog-body">
        <dl class="pl-list-form">
            <dt><label class="">아이디</label></dt>
            <dd>
                <v-text-field class="pl-form is-lg" placeholder="아이템명" v-model="USER_ID" maxLength="18" disabled></v-text-field>                
            </dd>
            <dt><label class="is-required">기존 비밀번호</label></dt>
            <dd>
              <v-text-field type="password" class="pl-form is-lg" placeholder="기존 비밀번호" v-model="PASSWORD_OLD" maxLength="18"></v-text-field>  
            </dd>
            <dt><label class="is-required">새 비밀번호</label></dt>
            <dd>
              <v-text-field type="password" class="pl-form is-lg" placeholder="새 비밀번호" v-model="PASSWORD_NEW" maxLength="18"></v-text-field>  
            </dd>
            <dt><label class="is-required">새 비밀번호 재확인</label></dt>
            <dd>
              <v-text-field type="password" class="pl-form is-lg" placeholder="새 비밀번호 재확인" v-model="PASSWORD_NEW2" maxLength="18"></v-text-field>  
            </dd>
        </dl>
        <div class="pl-info-txt is-blue is-mt-10">특수문자는 포함 8자리 이상</div>
        <div class="pl-info-txt is-blue">특수문자는 ‘ !, @, #, -$, & ’만 사용 가능</div>
      </div>
      <div class="pl-dialog-footer">
        <v-btn class="pl-btn is-lg" @click="checkdialogFindPass">취소</v-btn>
        <v-btn class="pl-btn is-lg is-main" @click="checkPassword">비밀번호 변경</v-btn>
      </div>
    </v-dialog>
    <!--// 비밀번호 변경 모달 -->
  </div>
...
export default {
  ...
  watch: {
    loginResult(value) {
      ...
      if (value !== null) {
        if (value > 0) {
          this.common_toast("로그인 되었습니다.", "success");
          this.$router.push("/home");
        } else if (value === 0) {
          this.common_toast("로그인을 실패하였습니다.", "error");
        } else {
          //로그인 실패
          this.common_toast("로그아웃을 실패하였습니다.", "success");
        }
      } else {
        //로그아웃 성공
        this.common_toast("로그아웃 되었습니다.", "success");
      }
    },
  },
  methods: {
    ...
    changePassword() {
      ...
      api
        .post("api/login/password/modify", {
    ...
    getCompany() {
      this.$store
        .dispatch("userStore/AC_USER_COMPANY", {
          user_id: this.uid,
        })
        .then((response) => {
          //사용자 상세정보 조회
